...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      switch (activeTab) {
        case 'communities':
          url = 'http://app.dms-api.com/v1/communities';
          break;
        case 'category':
          url = 'http://app.dms-api.com/v1/disability-categories';
          break;
        case 'type':
          if (selectedCategory) {
            url = `http://app.dms-api.com/v1/disability-types/category/${selectedCategory}`;
          } else {
          ...
          break;
        case 'assistance':
          url = 'http://app.dms-api.com/v1/assistance-types';
          break;
        case 'gender':
          setData(prev => ({
            ...prev,
  ...
  const handleAdd = async (tabKey, value) => {
    ...
    switch (tabKey) {
      ...
      case 'type':
        ...
        url = 'http://app.dms-api.com/v1/disability-types';
  ...
  const handleModalSave = async (newValue, newCategoryId) => {
    ...
    if (activeTab === 'type') {
      url = `http://app.dms-api.com/v1/disability-types/${id}`;
      body = {};
      if (newCategoryId) body.category_id = Number(newCategoryId);
      if (newValue) body.type_name = newValue;
    } else if (activeTab === 'communities') {
      url = `http://app.dms-api.com/v1/communities/${editItem.community_id || editItem.id}`;
      body = { community_name: newValue };
    } else if (activeTab === 'category') {
      url = `http://app.dms-api.com/v1/disability-categories/${editItem.category_id || editItem.id}`;
      body = { category_name: newValue };
    } else if (activeTab === 'assistance') {
      url = `http://app.dms-api.com/v1/assistance-types/${editItem.assistance_type_id || editItem.id}`;
  ...
  const handleDelete = async (item) => {
    ...
    if (confirmed.isConfirmed) {
      ...
      switch (activeTab) {
        case 'communities':
          url = `http://app.dms-api.com/v1/communities/${id}`;
          break;
        case 'category':
          url = `http://app.dms-api.com/v1/disability-categories/${id}`;
          break;
        case 'type':
          url = `http://app.dms-api.com/v1/disability-categories/${selectedCategory}/types/${id}`;
          break;
        case 'assistance':
          url = `http://app.dms-api.com/v1/assistance-types/${id}`;
          break;
        default:
          return;
      }
