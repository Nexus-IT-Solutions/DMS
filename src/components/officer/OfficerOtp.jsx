...
const OfficerOtp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('https://app.dms-api.com/v1/users/password/verify-otp', { otp: otpValue });
  ...
  const handleResend = async (e) => {
    ...
    try {
      const response = await axios.post('https://app.dms-api.com/v1/users/password/verify-otp', { resend: true });
      const result = response.data;
      if (result.status === 'success') {
        setStatus('resent');
